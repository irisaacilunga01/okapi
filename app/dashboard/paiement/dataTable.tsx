...
import { Paiement as Payment } from "@/lib/types";
...
import { deletePaiement, deleteUser } from "@/lib/actions";
    ...
    cell: ({ row }) => {
      return (
        <div className="capitalize">
          {row.original.date.toLocaleDateString("fr-FR", {
            month: "short",
            day: "numeric",
            year: "numeric",
          })}
    ...
    cell: ({ row }) => {
      return (
        <Dialog>
            ...
            <DropdownMenuContent align="end" className="pr-10 py-2">
              ...
              <DropdownMenuItem>
                <Link
                  ...
                  href={`/dashboard/paiement/ajouter`}
                >
              ...
              <DropdownMenuItem>
                <Link
                  className="flex gap-3"
                  href={`/dashboard/paiement/${row.original.id}`}
                >
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle> Suppression paiement</DialogTitle>
              <DialogDescription>
                Etes-vous sur de supprimer {row.original.id}
                définitivement?
              </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter className="justify-end">
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    await deletePaiement(row.original.id);
                    toast({
                      title: "Supprimer",
                      description: `le paiement ${row.original.id}  a été supprimer avec succès`,
    ...
    <div className="w-full px-4 pt-4">
      ...
      <div className="flex items-center justify-end space-x-2 py-4">
        <div className="flex-1 text-sm text-muted-foreground">
          Total : {table.getFilteredRowModel().rows.length} clients.
        </div>
